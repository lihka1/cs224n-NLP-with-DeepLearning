...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in range(len(sentence)):
            ...
            if i-window_size < 0:
                window += start
                window += [feature for word in sentence[:i] for feature in word]
            else:
                window += [feature for word in sentence[i-window_size:i] for feature in word]
            window += curr_word
            if i+window_size >= len(sentence):
                window += [feature for word in sentence[i+1:] for feature in word]
                window += end
            else:
                window += [feature for word in sentence[i+1:i+window_size+1] for feature in word]

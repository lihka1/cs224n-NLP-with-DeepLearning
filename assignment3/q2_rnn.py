...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) >= max_length:ret.append((sentence[:max_length],labels[:max_length],[True]*max_length))
        else:
            sentence1 = sentence[:]
            labels1 = labels[:]
            mask = [True]*len(sentence)
            for i in range(len(sentence1), max_length):
                sentence1 += [zero_vector]
                labels1 += [zero_label]
                mask += [False]
            ret.append((sentence1[:max_length],labels1[:max_length],mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable("b2",shape=(self.config.n_classes,),
                             initializer=tf.contrib.layers.xavier_initializer())
        #h = tf.Variable(tf.zeros(),dtype=tf.float32)

...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        o_t,state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)
    ...
    def add_training_op(self, loss):
        ...
        grads = [pair[0] for pair in grad_var_pairs]
        var_s = [pair[1] for pair in grad_var_pairs]
        self.grad_norm = tf.global_norm(grads)
        if self.config.clip_gradients:
            clipped_grads,self.grad_norm = tf.clip_by_global_norm(grads,self.config.max_grad_norm)
            train_op = optimizer.apply_gradients(zip(clipped_grads,var_s))
        else:
            train_op = optimizer.apply_gradients(zip(grads,var_s))

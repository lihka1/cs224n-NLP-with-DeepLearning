class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA":
                ele_removed = self.stack[-2]
                self.stack.remove(ele_removed)
                self.dependencies.append((self.stack[-1],ele_removed))
        if transition == "RA":
                ele_removed = self.stack[-1]
                self.stack.remove(ele_removed)
                self.dependencies.append((self.stack[-1],ele_removed))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    #print model.predict(partial_parses)[0]
    while len(unfinished_parses) > 0: 
        required = []
        for parse in unfinished_parses:
            if len(parse.stack) > 1 or len(parse.buffer) > 0:
                required.append(parse)
        unfinished_parses = required
        if (len(unfinished_parses) == 0):break
        
        minibatch = unfinished_parses[0:batch_size]
        transitions = model.predict(minibatch)
        #print transitions,minibatch
        for partial_parse,transition in zip(minibatch, transitions):
#            print len(minibatch),len(transitions),"checkkkkkk"
#            print len(partial_parse.buffer)
#            print len(partial_parse.stack)
#            print partial_parse.stack,transition
